...
translator = Translator(
    proxies={"https": SyncHTTPProxy((b"http", b"127.0.0.1", 7890, b""))},
)
...
def pull_fl_explanation(extra_data, pos):
    ...
    if definitions is not None and len(definitions) > pos:
        ...
        for definition in definitions[pos][1]:
            ...
            tmp = None
            if len(definition) > 2 and definition[2] is not None:
                text += f"\n<i>{definition[2]}</i>"
                tmp = translator.translate(definition[2], src="en", dest="zh-cn").text

            foreign_language_explanation_details += text
            foreign_language_explanation_details += "<br>"
            foreign_language_explanation_details += translator.translate(
                definition[0], src="en", dest="zh-cn"
            ).text
            if tmp is not None:
                foreign_language_explanation_details += f"\n<i>{tmp}</i>"
...
def pull_yl_explanation(extra_data, pos):
    ...
    if all_translations is not None and len(all_translations) > pos:
        ...
        your_language_explanation_details = ", ".join(
            [translation for translation in all_translations[pos][1]]
        )

        return [your_language_explanation_word_type, your_language_explanation_details]
    else:
        return ["", ""]
...
def translate_word(word, numtrans, src="auto", dest="en"):
    ...
    ] = pull_fl_explanation(extra_data, 0)
    [
        foreign_language_explanation_word_type_2,
        foreign_language_explanation_details_2,
    ] = pull_fl_explanation(extra_data, 1)
    [
        foreign_language_explanation_word_type_3,
        foreign_language_explanation_details_3,
    ] = pull_fl_explanation(extra_data, 2)

    [your_language_explanation_word_type_1, your_language_explanation_details_1] = (
        pull_yl_explanation(extra_data, 0)
    )
    [your_language_explanation_word_type_2, your_language_explanation_details_2] = (
        pull_yl_explanation(extra_data, 1)
    )
    [your_language_explanation_word_type_3, your_language_explanation_details_3] = (
        pull_yl_explanation(extra_data, 2)
    )
...
if __name__ == "__main__":
    print(translate_word("forge", 3, "en", "zh-cn"))
