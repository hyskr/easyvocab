...
def process_word(col, deck, word, options, progress):
    ...
    if options and options.get("tts"):
        ...
        try:
            ...
            print(f"Warning - failed to generate tts for {question}")
...
def process_words(col, deck, words, options, progress, dialogs, event):
    ...
    for word in words:
        ...
        if process_word(col, deck, word, options, progress(word, i + 1, total)):
            translated += 1
        i += 1
...
def wire_buttons(dialogs, col):
    def wrap(future):
        dialogs["progress"].ui.buttonBox.button(
...
def generate_cards(col, deck, text, options, dialogs):
    ...
    future = mw.taskman.run_in_background(
        util.wrap_nonary(process_words)(
            col, deck, text, options, progress, dialogs, event
        ),
        wire_buttons(dialogs, col),
    )

    # Set up cancel button to cancel card generation
    dialogs["progress"].ui.buttonBox.rejected.connect(
        cancel(dialogs, future, event, col)
    )
